...
const expectedTokenHash = "c7ba4c3d3dbceb8c43491819ecbc9982f3151b05898e821e314511b890fc76a3";
...
export async function POST(request: Request) {
  ...
  const preview = renderMonthlyWatchlistEmail({
    month: 9,
    movies: await sampleMovies(tmdbToken),
    sendDate: "2026-09-01",
    unsubscribeUrl: absoluteUrl("/profile/edit"),
    year: 2026,
  });
  const sampleBanner = '<div style="background:#facc15;border-radius:12px;color:#020617;font-size:12px;font-weight:900;letter-spacing:.1em;margin:0 auto 18px;max-width:620px;padding:12px;text-align:center">UPDATED DESIGN PREVIEW — SAMPLE MOVIE CONTENT</div>';
  const html = preview.html.replace('<span style="display:none!important;', `${sampleBanner}<span style="display:none!important;`);
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: {
      Authorization: `Bearer ${resendApiKey}`,
      "Content-Type": "application/json",
      "Idempotency-Key": "monthly-watchlist-design-preview-no-badges-2026-09-05",
    },
    body: JSON.stringify({
      from,
      html,
      subject: `[UPDATED TEST PREVIEW] ${preview.subject}`,
      text: `UPDATED DESIGN PREVIEW — SAMPLE MOVIE CONTENT\n\n${preview.text}`,
      to: email,
    }),
  });
